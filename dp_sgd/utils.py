...
def SoftThreshold(t, threshold_ratio, name=None):
  ...
  with tf.op_scope([t, threshold_ratio], name, "soft_thresholding") as name:
    saved_shape = tf.shape(t)
    t2 = tf.reshape(t, tf.concat([tf.slice(saved_shape, [0], [1]), -1], 0))
    t_abs = tf.abs(t2)
...
def isclose(a, b, rel_tol=1e-09, abs_tol=0.0):
    return abs(a-b) <= 1e-09 #max(rel_tol * max(abs(a), abs(b)), abs_tol)

def getIndexAndPrevProb(prob, predArray): #todo: a better way to know its predicted position?
    sortedArray = sorted(predArray)[::-1]
    prevProb = 0
    for i in range(len(sortedArray)):
        if isclose(prob, sortedArray[i]):
            return i, prevProb
        prevProb += sortedArray[i]
    return -1

def isDetectedNormal(pos, prevProb, bTopK, topK, topPerc):
    print('pos, prevProb, bTopK, topK, topPerc', pos, prevProb, bTopK, topK, topPerc)
    if bTopK and pos<topK or not bTopK and prevProb<topPerc:
        return True
    else:
        # print('detected anomaly, hist', hist, 'pos', pos)
        return False
